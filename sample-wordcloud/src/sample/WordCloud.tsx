...
class WordCloud extends Component<WordCloudProps> {
    ...
    private resetCanvas = () => {
        ...
        ctx.clearRect(0,0,this.originalCanvasWidth,this.originalCanvasHeight);
    }
    ...
    onFinished = () => {
        ...
        this.createWord(ctx, canvas);
    };

    @action
    private createWord = (ctx: CanvasRenderingContext2D, canvas: any) => {
        ...
        wordData.map((w, idx) => {
            ...
            if (idx % 2 === 0) {
                ...
                if (isFind) {
                    canvasBitmap = this.drawWord(canvas, ctx, w, x, y, 'sero');
                    resultList.push({
                        x,
                        y,
                        word: w.word,
                ...
                if (isFind) {
                    canvasBitmap = this.drawWord(canvas, ctx, w, x, y, 'garo');
                    resultList.push({
                        x,
                        y,
                        word: w.word,
            ...
            if (JSON.stringify(beforeCanvasMap) === JSON.stringify(canvasBitmap)) {
                const {
                    ...
                    direction,
                    isFind
                } = this.putWordGap(canvas, ctx, w, canvasBitmap, idx, textHeightArea, textWidthArea);
                canvasBitmap = Bitmap;
                if (isFind) {
                    resultList.push({
                        x,
                        ...
                        {this.resultList.map((d, i) => {
                            ...
                            const fontsize = Math.max(d.size * this.scale, 10);
                                ...
                                return <text key={`word-${i}`}
                                      ...
                                      dominantBaseline={`${d.direction === 'sero' ? "text-after-edge" : "hanging"}`}
